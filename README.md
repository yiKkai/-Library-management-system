...
![登录流程图](src/%E7%99%BB%E5%BD%95%E6%B5%81%E7%A8%8B%E5%9B%BE%E7%89%87.png)
...
统计模块：
1.热门图书排行榜
热门图书排行榜可以显示目前书库中所有图书的被借阅次数，根据借阅次数来对热门书籍进行排名，选出借阅次数最高的书籍为最热门图书
该功能对图书馆进书决策人员以及读者有一个参考作用，根据最热门的书籍来决定入库/借阅哪些书
管理员用户则可以对排行榜数据进行清空，则所有书籍被借阅的次数清零。
每当有读者再次借阅时，则会生成新的排行榜


2.读者排行榜
读者排行榜的主体功能是对所有读者的借阅次数进行统计，读取数据库中所有用户身份为读者的用户的借阅次数作为数据集，展示到图表中
管理员用户则可以对排行榜数据进行清空，则所有读者用户的借阅次数清零。
每当有读者再次借阅时，则会生成新的排行榜

3.图书分布统计
通过读取数据库中的所有书籍，根据每个书籍的所属类别进行分类，计算出每个类别的书籍在总书籍中的占比，得出图书分布信息
每当管理员用户对图书信息进行增加、删除、修改操作，图书分布统计也会产生相应的变化


4.借阅日期统计
读取借阅表中的所有借阅日期的信息，根据日期进行分组得出每日的借阅次数生成数据集，通过该数据集来生成每日借阅次数变动统计表
![输入图片说明](src/%E5%9B%BE%E7%89%87%E6%A8%A1%E5%9D%97%E5%9B%BE.png)

数据库：
在数据库中制作了3张主要的表格，分别为图书表、用户表、借阅记录表：


图书表：用于对图书信息进行相关修改、查询


借阅记录表：通过用户对图书的借阅产生相关借阅记录


用户表：用于对用户信息进行相关修改、查询
